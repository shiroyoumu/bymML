...
if __name__ == '__main__':
    ...
    trainSet = CollectTrainData(con, host1, series, itemNum)
    testSet = CollectTrainData(con, host2, series, itemNum)
    # 异常处理
    ...
    # 制作数据
    trainX, trainY = CollectAllData(trainSet3, max([step_tcn, step_lstm]))
    testX, testY = CollectAllData(testSet3, max([step_tcn, step_lstm]))
    # 整理格式
    ...
    model.fit(trainX, trainY, epochs=25, batch_size=64, verbose=1)
    # 对测试数据的Y进行预测
    ...
    # plt.figure(figsize=(480, 12))
    # plt.suptitle("host{} in {}".format(host2[0], name))
    #
    # a = np.full((step_tcn, testSet.shape[1]), np.nan)
    # b = np.concatenate([a, testSet], axis=0)
    # plt.plot(b.flatten(order='F'))
    #
